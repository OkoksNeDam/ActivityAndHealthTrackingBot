...
from app.database.requests import user_main_info_requests, user_water_intake_requests, \
    user_food_intake_requests, user_workout_info_requests
...
async def check_progress(message: Message):
    ...
    if not user_main_info_list:
        ...
        today_water_consumption = \
            await user_water_intake_requests.get_total_water_consumption(tg_id=message.from_user.id,
                                                                         date=datetime.now())
        ...
        today_calories_consumption = await user_food_intake_requests.get_calories_sum(tg_id=message.from_user.id,
                                                                                      date=datetime.now())
